...
namespace Controller
{
    public class UsuarioController
    {
        ...
        public void BuscarUsuario(string username,string email, string password)
        {
            try
            {
               SqlConnection cx = conexion.ObtenerConexion();
                string sql = @"SELECT * FROM Usuario WHERE nombre_Usuario = @Username OR contraseña_Usuario = @Password OR correo_Usuario = @Email";
                SqlCommand cmd = new SqlCommand(sql, cx);
